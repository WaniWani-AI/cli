...
| `waniwani connect` | Links the current repo to a managed or external MCP agent. Writes `waniwani.config.ts` |
...
2. `bunx @waniwani/cli@latest connect` — pick an org, pick or create an agent, writes `waniwani.config.ts` at the repo root.
...
- **`waniwani.config.ts`** — project bindings (`orgId`, `projectId`), optional `apiKey`, `apiUrl`, `devPort`. Created by `waniwani connect`. Shared in shape with `@waniwani/sdk`.
...
2. `apiKey` in `waniwani.config.ts`
...
- **Expecting `.waniwani/` to be committed** — it stores OAuth tokens. Add it to `.gitignore`. `waniwani.config.ts` *is* meant to be committed (it's just project bindings, no secrets).
...
- **Re-running `waniwani connect` after writing `waniwani.config.ts` by hand** — it'll prompt to switch projects. If your hand-written config is correct, decline; the existing binding stays.
